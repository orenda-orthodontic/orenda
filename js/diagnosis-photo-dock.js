...
//   Intra-oral      -> Frontal + Right + Left + Occlusal Upper + Occlusal Lower + Panorama سوا
...
// بيبدأ "مطوي" (header بس ظاهر) — كان شغال بالعرض الكامل ومغطي عمود القيمة في شيت التشخيص
// (شكوى: "الصورة كبيرة، مبقتش شايف الكلام"). دلوقتي بتفتح بضغطة واحدة وقت المقارنة، وتتقفل
// بضغطة تانية، بدل ما تفضل واخدة مساحة طول الوقت.
let diagPhotoDockCollapsed = true;
...
function ensureDiagPhotoDock(){
  ...
  if(!dock){
    ...
    dock.style.cssText = 'position:fixed;bottom:20px;right:20px;width:260px;max-width:calc(100vw - 24px);max-height:60vh;background:var(--panel);border:1px solid var(--border);border-radius:12px;box-shadow:0 4px 18px rgba(0,0,0,.35);z-index:950;display:flex;flex-direction:column;overflow:hidden;';
