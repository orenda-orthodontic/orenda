...
// متابعة "مدفعش" بتبدأ من الشهر ده بس — أي فجوة دفع قبله ما بتتحسبش، عشان الحساب ميحاسبكش على
// حالات كانت أصلاً متأخرة في الدفع قبل ما الميزة دي تتضاف
const UNPAID_TRACKING_START_YM = '2026-09';
function computeUnpaidMonths(file){
  if(!file || (file.caseStatus || 'active') !== 'active') return 0;
  const paidMonths = new Set((file.payments||[]).filter(p=>p.date).map(p => p.date.slice(0,7)));
  const caseStartYm = (computeCaseStartDate(file) || '').slice(0,7);
  // effective start = الأحدث بين تاريخ بداية الحالة الفعلي وبداية المتابعة — أيهما جه بعد الآخر
  const startYm = caseStartYm && caseStartYm > UNPAID_TRACKING_START_YM ? caseStartYm : UNPAID_TRACKING_START_YM;
...
// ============ DEFAULT DATE FOR RECORDS THAT ARE NOT A VISIT THEMSELVES ============
// Photos, payments, extra charges, notes, a case debond/new-case date, a broken-bracket log...
// used to default to the real calendar day they were typed in. When that day isn't a real clinic
// visit day (the doctor is just catching up from home), it now defaults to the last real
// follow-up date logged for ANY patient of the same clinic instead — and every one of those
// date fields stays editable, so it's only a starting value. A monthly follow-up entry itself is
// NOT affected (it defines a visit day, so it keeps using today's date).
// "Real" = a dated monthly-log entry that isn't marked missed — same rule computeWorkDaysThisMonth uses.
function realVisitDatesOfFile(file){
  const out = [];
  ((file && file.monthlyLog) || []).forEach(e => {
    if(e && typeof e.date === 'string' && e.date && !e.missed) out.push(e.date);
  });
  return out;
}
function currentClinicRealVisitDates(){
  const cached = (state.clinicRealVisitDates && state.clinicRealVisitDates[state.currentClinicId]) || [];
  const set = new Set(cached);
  // the patient file open right now may hold entries newer than what the last clinic-wide scan saw
  realVisitDatesOfFile(state.currentPatientFile).forEach(d => set.add(d));
  return set;
}
function defaultRecordDate(){
  const today = todayStr();
  const dates = [...currentClinicRealVisitDates()].filter(d => d <= today).sort();
  if(!dates.length) return today; // nothing logged yet for this clinic — no better guess than today
  if(dates[dates.length - 1] === today) return today; // today IS a real visit day
  return dates[dates.length - 1];
}

// small note shown under a date field whose starting value came from defaultRecordDate() rather than today
function defaultDateHintHtml(dateStr){
  if(!dateStr || dateStr === todayStr()) return '';
  return '<div style="font-size:11px;color:var(--muted);margin-top:3px;">📅 ده تاريخ آخر متابعة فعلية في العيادة — عدّله لو مختلف</div>';
}
...
async function loadClinicPatientAggregates(clinicId, month){
  ...
  // P0-5: كل الجمع بالقروش (integer) والتحويل لجنيه مرة واحدة عند التخزين في state — مفيش float بيتراكم
  let totalCents = 0;
  ...
  const avatarMap = {}; // patientId -> "Frontal Rest (before)" photo slot, shown as the avatar in the patient list
  const visitDates = new Set(); // every real (non-missed) follow-up date across this clinic's patients
  function applyFile(patientId, file){
    ...
    payments.forEach(pay=>{
      if((pay.date||'').slice(0,7) === month){
        const amtCents = readAmountCents(pay);
        totalCents += amtCents;
        yourShareCents += Math.round(amtCents * patientPct / 100); // نفس تقريب finance-tab.js: مرة لكل دفعة
      }
    });
    missedStatus[patientId] = computeMissedMonths(file);
    unpaidStatus[patientId] = computeUnpaidMonths(file);
    realVisitDatesOfFile(file).forEach(d => visitDates.add(d));
    const frontalRest = file && file.photos && file.photos.before && file.photos.before.extraoral && file.photos.before.extraoral.frontalRest;
    if(frontalRest && (frontalRest.path || frontalRest.url)) avatarMap[patientId] = { path: frontalRest.path, url: frontalRest.url };
  ...
  // one batched signing request for every avatar in this clinic (already-cached ones are skipped);
  // if signing fails the list simply falls back to the first-letter avatar
  try{ await refreshPhotoUrlCache(Object.values(avatarMap).map(a => a.path)); }catch(e){ console.error('avatar url signing failed', e); }
  if(!state.clinicRealVisitDates) state.clinicRealVisitDates = {};
  state.clinicRealVisitDates[clinicId] = [...visitDates].sort();
  state.clinicPatientAvatars = avatarMap;
  state.clinicSummaryTotal = centsToEgp(totalCents);
  state.clinicSummaryYourShare = centsToEgp(yourShareCents);
  state.clinicSummaryTotalCents = totalCents;
  state.clinicSummaryYourShareCents = yourShareCents;
...
function computeCaseStatsFromFiles(files, month, clinicCommissionPct){
  ...
  const monthTotalsCents = {}; allMonths.forEach(m=>monthTotalsCents[m]=0); // P0-5: التجميع بالقروش، monthTotals بتتملى من الـ cents في الآخر
  const monthYourShareCents = {}; allMonths.forEach(m=>monthYourShareCents[m]=0);
  ...
  files.forEach(({patient, file})=>{
    ...
    function sumForMonthCents(m){ return payments.filter(p=>(p.date||'').slice(0,7)===m).reduce((s,p)=>s+readAmountCents(p),0); }
    const thisCents = sumForMonthCents(month);
    const prevCents = monthsBack.map(sumForMonthCents);
    allMonths.forEach((m,idx)=>{
      const c = (idx===0 ? thisCents : prevCents[idx-1]);
      monthTotalsCents[m] += c;
      monthYourShareCents[m] += Math.round(c * patientPct / 100);
    });
    const thisAmt = centsToEgp(thisCents);
    const prevAmts = prevCents.map(centsToEgp);
    ...
    if(caseValueCents > 0 && paidSumCents >= caseValueCents) paidOffStillActive.push({ name: patient.name, remaining: centsToEgp(caseValueCents - paidSumCents) });
  ...
  allMonths.forEach(m=>{
    monthTotals[m] = centsToEgp(monthTotalsCents[m]);
    monthYourShareTotals[m] = centsToEgp(monthYourShareCents[m]);
  });
  return {
...
async function scanAllClinicsForAlerts(){
  ...
  const unpaidAlerts = [];
  const caseAlerts = [];
  const clinicCaseCounts = {};
  const financeBreakdown = {}; // clinicId -> {clinicName, caseValueCents, collectedCents, remainingCents, patients:[...]}
  const patientsIndex = []; // Global Search (V2.8) — every patient across every clinic
  let financeCaseValueCents = 0, financeCollectedCents = 0, financeRemainingCents = 0, financeOverdueCents = 0;
  await Promise.all((state.clinics||[]).map(async (c) => {
    financeBreakdown[c.id] = { clinicName: c.name, caseValueCents: 0, collectedCents: 0, remainingCents: 0, patients: [] };
    try{
      const patients = await getData('patients:' + c.id, []);
      clinicCaseCounts[c.id] = 0;
      if(!patients.length) return;
      patients.forEach(p => patientsIndex.push({ id: p.id, name: p.name, clinicId: c.id, clinicName: c.name }));
      const files = await batchLoadPatientFiles(patients);
      files.forEach(({patient, file})=>{
        if(!file) return;
        const active = (file.caseStatus || 'active') === 'active';
        if(active) clinicCaseCounts[c.id] += 1;
        ...
        const isUnpaidAlert = unpaid >= UNPAID_MONTHS_ALERT_THRESHOLD;
        if(isUnpaidAlert){
        ...
        if(active){
          ...
          // Finance summary + per-patient breakdown (V2.7 Dashboard): TOTAL CASE VALUE / COLLECTED
          // / REMAINING / OVERDUE, plus a 🟢 Paid / 🟠 Partial / 🔴 Overdue status per patient for
          // the Clinic → Patient → Payment history drill-down. Cents-based throughout.
          const caseValueCents = readFinanceTotalCents(file) + (file.financeExtras||[]).reduce((s,e)=>s+readAmountCents(e), 0);
          const collectedCents = (file.payments||[]).reduce((s,p)=>s+readAmountCents(p), 0);
          const remainingCents = Math.max(0, caseValueCents - collectedCents);
          financeCaseValueCents += caseValueCents;
          financeCollectedCents += collectedCents;
          financeRemainingCents += remainingCents;
          if(isUnpaidAlert) financeOverdueCents += remainingCents;

          const status = isUnpaidAlert ? 'overdue' : (remainingCents <= 0 ? 'paid' : 'partial');
          financeBreakdown[c.id].caseValueCents += caseValueCents;
          financeBreakdown[c.id].collectedCents += collectedCents;
          financeBreakdown[c.id].remainingCents += remainingCents;
          financeBreakdown[c.id].patients.push({ patientId: patient.id, name: patient.name, caseValueCents, collectedCents, remainingCents, status });
        }
      });
    }catch(e){
  ...
  state.globalClinicCaseCounts = clinicCaseCounts;
  state.globalFinanceSummary = {
    caseValueCents: financeCaseValueCents,
    collectedCents: financeCollectedCents,
    remainingCents: financeRemainingCents,
    overdueCents: financeOverdueCents
  };
  Object.values(financeBreakdown).forEach(cb => {
    cb.patients.sort((a, b) => b.remainingCents - a.remainingCents);
  });
  state.globalFinanceBreakdown = financeBreakdown;
  state.globalPatientsIndex = patientsIndex;
}
