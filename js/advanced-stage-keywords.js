...
async function handleFiring(file, e, entryId, doneUpper, doneLower, doneBoth){
  const { upper, lower } = detectStageSides(doneUpper, doneLower, doneBoth, FIRING_RE);
  let changed = false;
  if(upper && !e.firingUpperHandled){
    addStageTag(e, 'firing_upper');
    e.firingUpperHandled = true;
    changed = true;
  }
  if(lower && !e.firingLowerHandled){
    addStageTag(e, 'firing_lower');
    e.firingLowerHandled = true;
    changed = true;
  }
  if(!changed) return;
  await savePatientFile(state.currentPatientId, stripHelperFields(file));
  toast('اتسجلت Firing في مراحل العلاج');
  render();
}
...
async function scanAdvancedStages(entryId, doneUpper, doneLower, doneBoth){
  ...
  if(FIRING_RE.test(combined) && !e.firingHandled){
    await handleFiring(file, e, entryId, doneUpper, doneLower, doneBoth);
  }
}
