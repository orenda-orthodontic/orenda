...
function getDiagnosisSectionByTitle(file, title){
  return (file.diagnosisSections||[]).find(s => (s.title||'').trim().toLowerCase() === title.trim().toLowerCase());
}

function getNonEmptyDiagnosisFields(section){
  if(!section) return [];
  const out = [];
  (section.fields||[]).forEach(f=>{ if((f.label||'').trim() || (f.value||'').trim()) out.push(f); });
  (section.groups||[]).forEach(g=>{
    (g.fields||[]).forEach(f=>{
      if((f.label||'').trim() || (f.value||'').trim()) out.push({...f, label:`${g.title} · ${f.label}`});
    });
  });
  return out;
}

function renderClinicalPlanBoard(file){
  const problemSection = getDiagnosisSectionByTitle(file, 'Problem List and Mechanics');
  const planSection = getDiagnosisSectionByTitle(file, 'Treatment Plan');
  const problems = getNonEmptyDiagnosisFields(problemSection).filter(f=>f.label || f.value);
  const plan = getNonEmptyDiagnosisFields(planSection).filter(f=>f.label || f.value);
  const status = file.clinicalPlanStatus || 'in-progress';
  const nextDecision = file.clinicalNextDecision || '';
  ...
  const problemCards = problems.slice(0,6).map((f,i)=>`
    <div class="clinical-plan-item">
      <span class="clinical-plan-index">${i+1}</span>
      <div><b>${escapeHtml(f.label || 'مشكلة')}</b>${f.value ? `<small>${escapeHtml(f.value)}</small>` : ''}</div>
    </div>
  `).join('');
  const planRows = plan.slice(0,6).map(f=>`
    <div class="clinical-plan-row"><span>${escapeHtml(f.label || 'خطة')}</span><b>${escapeHtml(f.value || '—')}</b></div>
  `).join('');

  return `
    <section class="clinical-plan-board">
      <div class="clinical-plan-head">
        <div>
          <div class="eyebrow">CLINICAL COMMAND CENTER</div>
          <h2>من التشخيص إلى القرار</h2>
          <p>ملخص سريع للحاجات المهمة قبل ما تبدأ تعدّل في الحالة.</p>
        </div>
        <select id="clinicalPlanStatus" class="clinical-status-select">
          ${statusOptions.map(([v,l])=>`<option value="${v}" ${status===v?'selected':''}>${l}</option>`).join('')}
        </select>
      </div>

      <div class="clinical-flow">
        <div class="clinical-flow-block">
          <div class="clinical-flow-label">01 · PROBLEMS</div>
          <div class="clinical-plan-list">${problemCards || '<div class="clinical-plan-empty">لسه مفيش Problem List متسجلة.</div>'}</div>
        </div>
        <div class="clinical-flow-arrow">→</div>
        <div class="clinical-flow-block">
          <div class="clinical-flow-label">02 · TREATMENT PLAN</div>
          <div class="clinical-plan-rows">${planRows || '<div class="clinical-plan-empty">لسه مفيش Treatment Plan متسجل.</div>'}</div>
        </div>
      </div>

      <div class="clinical-next-decision">
        <div class="clinical-next-icon">→</div>
        <div class="clinical-next-body">
          <label for="clinicalNextDecision">03 · NEXT CLINICAL DECISION</label>
          <textarea id="clinicalNextDecision" placeholder="إيه أهم حاجة عايز تقيّمها أو تقررها في الزيارة القادمة؟">${escapeHtml(nextDecision)}</textarea>
          <small>مثال: Evaluate LL1 rotation before changing mechanics.</small>
        </div>
      </div>
    </section>
  `;
...
function renderDiagnosisTab(){
  ...
  return `
    <div class="section-title diagnosis-page-title">التشخيص وخطة العلاج</div>
    ${renderClinicalPlanBoard(file)}
    <div class="card diagnosis-chief-card" style="margin-bottom:14px;">
...
function attachDiagnosisHandlers(){
  ...
  const clinicalStatus = document.getElementById('clinicalPlanStatus');
  if(clinicalStatus) clinicalStatus.onchange = async () => {
    state.currentPatientFile.clinicalPlanStatus = clinicalStatus.value;
    await savePatientFile(state.currentPatientId, stripHelperFields(state.currentPatientFile));
    toast('اتحفظ');
  };
  const nextDecision = document.getElementById('clinicalNextDecision');
  if(nextDecision) nextDecision.onchange = async () => {
    state.currentPatientFile.clinicalNextDecision = nextDecision.value.trim();
    await savePatientFile(state.currentPatientId, stripHelperFields(state.currentPatientFile));
    toast('اتحفظ');
  };
}
...
function openEditPatientModal(){
  ...
  const currentStart = (pFile && computeCaseStartDate(pFile)) || '';
  const bg = document.createElement('div');
  bg.className = 'modal-bg';
  bg.innerHTML = `
    <div class="modal">
      ...
      <div class="field">
        <label>نوع البراكيت</label>
        <select id="editPatientBracketSystem">
          <option value="" ${!(pFile && pFile.bracketSystem)?'selected':''}>غير محدد</option>
          <option value="roth" ${pFile && pFile.bracketSystem==='roth'?'selected':''}>ROTH</option>
          <option value="mbt" ${pFile && pFile.bracketSystem==='mbt'?'selected':''}>MBT</option>
        </select>
      </div>
      <div class="field">
        <label>تاريخ بداية الحالة</label>
        <input type="date" id="editPatientCaseStart" value="${escapeHtml(currentStart)}">
        <div style="font-size:11px;color:var(--muted);margin-top:3px;">بيتحسب تلقائي من أول متابعة — لو غيّرته بيتثبت على اللي كتبته، ولو مسحته بيرجع تلقائي</div>
      </div>
      <div class="modal-actions">
  ...
  document.getElementById('saveEditPatientBtn').onclick = async () => {
    ...
    if(pFile){
      pFile.bracketSystem = document.getElementById('editPatientBracketSystem').value || null;
      const newStart = document.getElementById('editPatientCaseStart').value || '';
      if(newStart !== currentStart){
        // only touch the override when the date was actually changed — an untouched field must not
        // freeze the auto-computed date in place; clearing it hands control back to the auto value
        pFile.caseStartDateOverride = newStart || null;
      }
