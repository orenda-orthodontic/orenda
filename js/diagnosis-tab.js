    ...
    const bodyHtml = isOpen ? `
      <div class="diag-acc-body">
        ...
        <div class="row" style="flex-wrap:wrap;margin-top:10px;">
          ...
          ${s.title.trim().toLowerCase() === 'cephalometric' ? `<button class="small webceph-import-btn">📥 استيراد من WebCeph</button>` : ''}
...
function attachDiagnosisHandlers(){
  ...
  document.querySelectorAll('.webceph-import-btn').forEach(el=>{
    el.onclick = () => openWebcephImportModal();
  });
...
function openEditPatientModal(){
  ...
  bg.innerHTML = `
    <div class="modal">
      ...
      <div class="field">
        <label>المدة المتوقعة للعلاج بالشهور (اختياري)</label>
        <input type="number" id="editPatientExpectedDuration" min="1" value="${pFile && pFile.expectedDurationMonths ? pFile.expectedDurationMonths : ''}" placeholder="مثلاً 18">
        <div style="font-size:11px;color:var(--muted);margin-top:3px;">لو محدد، هيظهر في هيدر المريض زي "8 / 18 شهر" بدل "8 شهر" بس</div>
      </div>
      <div class="modal-actions">
  ...
  document.getElementById('saveEditPatientBtn').onclick = async () => {
    // snapshot القيم القديمة قبل التعديل عشان سجل العمليات يوضح "اتغير من إيه لإيه"
    const before = {
      name: patient.name, phone: patient.phone || '',
      bracket: (pFile && pFile.bracketSystem) || '',
      start: (pFile && pFile.caseStartDateOverride) || '',
      dur: (pFile && pFile.expectedDurationMonths) || ''
    };
    ...
    if(pFile){
      ...
      const expDurRaw = document.getElementById('editPatientExpectedDuration').value;
      pFile.expectedDurationMonths = expDurRaw ? (parseInt(expDurRaw, 10) || null) : null;
      await savePatientFile(state.currentPatientId, stripHelperFields(pFile));
    }
    const changes = [];
    if(before.name !== patient.name) changes.push(`الاسم: ${before.name} ← ${patient.name}`);
    if(before.phone !== (patient.phone || '')) changes.push(`التليفون: ${before.phone || '-'} ← ${patient.phone || '-'}`);
    if(pFile){
      if(before.bracket !== (pFile.bracketSystem || '')) changes.push(`نوع البراكيت: ${before.bracket || '-'} ← ${pFile.bracketSystem || '-'}`);
      if(before.start !== (pFile.caseStartDateOverride || '')) changes.push(`بداية الحالة: ${before.start || 'تلقائي'} ← ${pFile.caseStartDateOverride || 'تلقائي'}`);
      if(String(before.dur) !== String(pFile.expectedDurationMonths || '')) changes.push(`المدة المتوقعة: ${before.dur || '-'} ← ${pFile.expectedDurationMonths || '-'}`);
    }
    if(changes.length) await logActivity('edit_patient', `عدّل بيانات مريض ${patient.name} — ${changes.join(' | ')}`);
    bg.remove();
