...
function openEditPatientModal(){
  ...
  document.getElementById('saveEditPatientBtn').onclick = async () => {
    const before = { name: patient.name, phone: patient.phone || '', bracket: (pFile && pFile.bracketSystem) || '', start: currentStart };
    ...
    if(before.phone !== (patient.phone || '')) changes.push(`الموبايل: ${before.phone || '—'} ← ${patient.phone || '—'}`);
    if(pFile && before.bracket !== (pFile.bracketSystem || '')) changes.push(`البراكيت: ${before.bracket || '—'} ← ${pFile.bracketSystem || '—'}`);
    if(pFile){ const nowStart = computeCaseStartDate(pFile) || ''; if(before.start !== nowStart) changes.push(`بداية الحالة: ${before.start || '—'} ← ${nowStart || '—'}`); }
