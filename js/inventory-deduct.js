...
async function validateAndDeductBatch(list){
  ...
  // نفس الصنف ممكن يتكرر في القايمة (مثلاً إيلاستيك اتضاف مرتين) — التحقق لازم يكون على مجموع
  // المطلوب من كل صنف، مش على كل سطر لوحده، وإلا صنف كميته 1 يعدّي مرتين وينزل -1
  const totals = new Map();
  list.forEach(x => totals.set(x.item, (totals.get(x.item) || 0) + (x.qty || 1)));
  const short = [];
  totals.forEach((needed, item) => {
    if(!checkInventoryAvailable(item, needed)){
      const line = list.find(x => x.item === item);
      short.push({ item, qty: needed, label: line.label });
    }
  });
  if(short.length){
  ...
  // سجل العمليات (نفس زرار "سجل العمليات") — الخصم اتعمل في الذاكرة، والمستدعي هو اللي بيحفظ المخزون بعده
  if(typeof logActivity === 'function'){
    const lines = list.map(x => `${x.label || x.item.name} ×${x.qty || 1} (الكمية بقت ${x.item.qty})`).join('، ');
    await logActivity('inventory_deduct', `خصم من المخزن: ${lines}${typeof patientNameForLog === 'function' && state.currentPatientId ? ' — مريض ' + patientNameForLog() : ''}`);
  }
  return true;
}
