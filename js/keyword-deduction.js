...
async function scanAndDeductKeywords(entryId, text){
  ...
  // NOTE: wires and elastics used to be auto-detected from free text here (e.g. "wire 0.014",
  // a bare fraction size like "3/16"). Both moved to the explicit Wire/Elastic/Accessories
  // picker (see wire-accessories-modal.js) so a doctor phrasing it differently than the regex
  // expects can't silently skip the deduction. Bonding Upper's fixed 0.012 wire deduction is
  // unrelated to this and still happens automatically (bonding-newcase.js).
  ...
  // pass 1: apply the inventory-side deduction only, and save it — nothing else is committed
  // until we know this actually persisted (not rejected by a concurrency conflict / offline)
  ...
  if(!okInv){
    // فشل الحفظ أو تعارض — setData بيبقى وضح السبب في التوست بتاعه، والمخزون اترجع تلقائي لآخر
    // نسخة من السيرفر، فمنكملش نسجل أي حاجة تانية (كسر فص/رسوم/سجل استخدام) بناءً على خصم ملغي
    render();
    return;
  }

  // pass 2: now that the deduction is confirmed saved, record the patient-file-side effects
  candidates.forEach(c=>{
    const matUse = { id: uid(), itemId: c.item.id, itemName: c.item.name, itemType: c.item.type || '', qty: 1, matchKey: c.matchKey, source:'keyword' };
