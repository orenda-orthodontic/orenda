...
function computeActiveAlerts(){
  ...
  if(state.clinicMissedStatus){
    const clinic = state.clinics.find(c=>c.id === state.currentClinicId);
    Object.keys(state.clinicMissedStatus).forEach(pid=>{
      const missed = state.clinicMissedStatus[pid];
      if(missed >= MISSED_MONTHS_ALERT_THRESHOLD){
        const p = (state.patients||[]).find(x=>x.id===pid);
        if(p){
          alerts.push({ key: 'missed:' + pid, signature: String(missed), label: `⚠ ${p.name}${clinic?' — '+clinic.name:''} مجاش من ${missed} شهر` });
        }
      }
    });
  }
  if(state.clinicCaseAlerts){
    const clinic = state.clinics.find(c=>c.id === state.currentClinicId);
    Object.keys(state.clinicCaseAlerts).forEach(pid=>{
      const info = state.clinicCaseAlerts[pid];
      const p = (state.patients||[]).find(x=>x.id===pid);
      if(!p) return;
      if(info.oneYear){
        alerts.push({ key: 'oneyear:' + pid, signature: String(info.months), label: `🎂 ${p.name}${clinic?' — '+clinic.name:''} كمّل سنة في العلاج — يستاهل مراجعة` });
      }
      if(info.longDuration){
        alerts.push({ key: 'longcase:' + pid, signature: String(info.months), label: `⏳ ${p.name}${clinic?' — '+clinic.name:''} قاعد في العلاج من ${info.months} شهر (أكتر من سنة ونص)` });
      }
    });
  }
  return alerts;
