...
function globalSearchQuickActions(){
  return [
    ...
    { icon:'📓', label:'ملاحظاتي', run: async () => { closeGlobalSearchModal(); await loadOrthoNotes(); state.view='orthonotes'; render(); } }
  ];
}
...
function ensureTopNavBar(){
  ...
  const items = [
    ...
    { view: 'orthonotes', label: '📓 ملاحظاتي', active: state.view === 'orthonotes' },
  ];
  ...
  bar.querySelectorAll('[data-nav]').forEach(b => {
    b.onclick = async () => {
      ...
      } else if(view === 'orthonotes'){
        await loadOrthoNotes();
        state.view = 'orthonotes';
        render();
      } else {
        state.view = view;
        render();
...
async function goToLocation(loc){
  ...
  if(loc.view === 'patient'){
    ...
    // loadPatients وloadInventory مش معتمدين على بعض — تشغيلهم بالتوازي هنا بدل الانتظار لبعض
    // بيقصّر وقت فتح شاشة المريض تقريبًا للنص.
    await Promise.all([loadPatients(loc.currentClinicId), loadInventory()]);
...
async function render(){
  ...
  try{
    ...
    } else if(state.view === 'orthonotes'){
      app.innerHTML = renderOrthoNotesView();
      attachOrthoNotesHandlers();
    }
    ensureNotificationBell();
    ensureTopNavBar();
