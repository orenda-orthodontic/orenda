...
function computeStageCounts(file){
  const counts = {};
  allTreatmentStages().forEach(st => counts[st.id] = []);
...
function renderStagesTab(){
  ...
  // order is preserved.
  const orderedStages = [...allTreatmentStages()].sort((a, b) => {
...
const TOOTH_SHAPE_PATHS = {
  incisor:  'M13,4 Q8,10 8,18 L8,35 Q8,41 14,43 L26,43 Q32,41 32,35 L32,18 Q32,10 27,4 Q20,1 13,4 Z',
  canine:   'M13,4 Q9,9 9,17 L9,29 Q9,33 12,36 L20,47 L28,36 Q31,33 31,29 L31,17 Q31,9 27,4 Q20,1 13,4 Z',
  premolar: 'M10,5 Q6,10 6,18 L6,31 Q6,36 10,39 Q14,42 17,38 Q20,35 23,38 Q26,42 30,39 Q34,36 34,31 L34,18 Q34,10 30,5 Q20,1 10,5 Z',
  molar:    'M5,6 Q2,11 2,19 L2,31 Q2,36 6,39 Q10,42 13,39 Q17,36 20,39 Q23,36 27,39 Q30,42 34,39 Q38,36 38,31 L38,19 Q38,11 35,6 Q20,2 5,6 Z'
};
...
function ensureToothShapeStyles(){
  ...
  st.textContent = `
    ...
    .tooth.tooth-shaped.selected .tsvg path{fill:#8ec1f0;stroke:#2b6cb0;stroke-width:3;}
    ...
    /* الشاشة المدموجة: المتابعة الشهرية فوق (الجزء الأكبر ٧٠٪) + خريطة الفصوص تحتها (٣٠٪) —
       عمود واحد دايمًا (مش جنب بعض)، على الموبايل والديسكتوب على حد سواء. */
    .monthly-split{display:flex;flex-direction:column;gap:16px;}
    .monthly-split-main{min-width:0;}
    .monthly-split-side{min-width:0;border-top:1px solid var(--border);padding-top:14px;
      max-height:46vh;overflow-y:auto;}
    .monthly-split-side .tooth-row{gap:4px;}
    .monthly-split-side .tooth.tooth-shaped{flex:1 1 0;width:auto;min-width:0;max-width:46px;}
    .monthly-split-side .tooth.tooth-shaped .tnum{font-size:11px;}
    .monthly-split-side .quad-gap{flex:0 0 12px;}

    /* نافذة اختيار الأسنان (Rebonding / De-rotation / Canine) — نفس شكل شارت المخزون */
    .pick-teeth .tooth-row{gap:4px;}
    .pick-teeth .tooth.tooth-shaped{flex:1 1 0;width:auto;min-width:0;max-width:44px;}
    .pick-teeth .tooth.tooth-shaped .tnum{font-size:10.5px;}
    .pick-teeth .quad-gap{flex:0 0 14px;}
  `;
...
function attachBracketsHandlers(){
  document.querySelectorAll('.tooth:not(.pick-tooth)').forEach(el=>{
