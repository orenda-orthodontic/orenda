...
function getPreviousMonthlyEntry(file, currentId){
  return [...(file.monthlyLog||[])]
    .filter(x => x.id !== currentId && x.date)
    .sort((a,b)=>(b.date||'').localeCompare(a.date||''))[0] || null;
}

function monthlyClinicalStatus(e){
  if(e.missed) return {cls:'missed', label:'العيان مجاش'};
  if((e.decision||'').trim() || (e.nextObjective||'').trim()) return {cls:'decision', label:'قرار سريري مسجل'};
  if((e.doneUpper||e.doneLower||e.doneBoth||e.done||'').trim()) return {cls:'documented', label:'الزيارة متسجلة'};
  return {cls:'draft', label:'لسه محتاجة تسجيل'};
}

function renderMonthlyClinicalCommand(e, file){
  const prev = getPreviousMonthlyEntry(file, e.id);
  const status = monthlyClinicalStatus(e);
  const previousText = prev
    ? [prev.decision, prev.nextObjective, prev.doneUpper, prev.doneLower, prev.doneBoth || prev.done]
        .filter(Boolean).join(' · ')
    : 'دي أول متابعة مسجلة للحالة';
  return `
    <div class="monthly-command">
      <div class="monthly-command-head">
        <div>
          <div class="monthly-command-kicker">CLINICAL VISIT</div>
          <h3>تفكير الزيارة</h3>
        </div>
        <span class="monthly-status ${status.cls}">${status.label}</span>
      </div>
      <div class="monthly-flow">
        <div class="monthly-flow-card">
          <span class="monthly-flow-icon">01</span>
          <div><b>ماذا لاحظت؟</b><small>Clinical observation</small></div>
          <textarea class="monthly-observation" data-id="${e.id}" placeholder="مثلاً: الـ alignment اتحسن، لكن LL1 لسه rotated...">${escapeHtml(e.observation||'')}</textarea>
        </div>
        <div class="monthly-flow-card">
          <span class="monthly-flow-icon">02</span>
          <div><b>ماذا قررت؟</b><small>Clinical decision + why</small></div>
          <textarea class="monthly-decision" data-id="${e.id}" placeholder="مثلاً: هنكمل نفس mechanics لأن...">${escapeHtml(e.decision||'')}</textarea>
        </div>
        <div class="monthly-flow-card">
          <span class="monthly-flow-icon">03</span>
          <div><b>الهدف القادم</b><small>Next clinical objective</small></div>
          <textarea class="monthly-next-objective" data-id="${e.id}" placeholder="مثلاً: Evaluate LL1 rotation before changing mechanics...">${escapeHtml(e.nextObjective||'')}</textarea>
        </div>
      </div>
      <div class="monthly-previous">
        <span>آخر سياق:</span>
...
function renderMonthEntry(e, opts={}){
  ...
  const fileForCommand = state.currentPatientFile || {monthlyLog:[]};
  const commandHtml = renderMonthlyClinicalCommand(e, fileForCommand);
...
function attachMonthlyHandlers(){
  ...
  ['observation','decision','nextObjective'].forEach(field=>{
    document.querySelectorAll(`.monthly-${field.replace(/[A-Z]/g, m => '-' + m.toLowerCase())}`).forEach(el=>{
      el.onchange = () => updateMonthlyEntry(el.dataset.id, {[field]: el.value});
    });
  });
