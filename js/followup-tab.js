...
async function addMaterialUse(entryId){
  ...
  // P0-4: تحقق من التوفر الأول — لو الكمية مش كافية مفيش حاجة بتتخصم ولا بتتسجل (بدل Math.max(0,...)
  // اللي كان بينزّل صفر بصمت ويسجل استخدام أكبر من اللي اتخصم فعلًا)
  if(!(await validateAndDeductBatch([{ item, qty, label: item.name }]))) return;

  const newUse = { id: uid(), itemId: item.id, itemName: item.name, itemType: item.type || '', qty };
  e.materialsUsed.push(newUse);
  ...
  await savePatientFile(state.currentPatientId, stripHelperFields(file));
  await logActivity('material_use', `خصم ${qty} من "${item.name}" (المخزون بقى ${item.qty}) لمريض ${patientNameForLog()}`);
