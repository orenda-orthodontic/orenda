  ...
  return `
    <div class="month-entry" data-entry-id="${e.id}">
      ...
      <div class="row" style="margin-bottom:8px;flex-wrap:wrap;gap:6px;">
        ...
        <button type="button" class="secondary small bracket-action-btn" data-id="${e.id}">🔩 Bracket</button>
        <button type="button" class="secondary small bonding-upper-action-btn" data-id="${e.id}">🆕 Bonding Upper</button>
        <button type="button" class="secondary small bonding-lower-action-btn" data-id="${e.id}">🆕 Bonding Lower</button>
  ...
  const keywordsBody = `
    <div class="card" style="background:#fafaf5;margin-bottom:14px;">
      ...
      <div style="font-size:12.5px;line-height:1.9;color:var(--muted);">
        <div><b>ريفيل/فص، براكيت، وأي قسم تاني في المخزون:</b> دلوقتي بزراير مش بكتابة كلمة في النص — "🦷 Rebonding / فص"، "🔩 Bracket" ثابتين، وكل قسم مخزون تاني عندك (زي "أوتاي") ليه زراره الخاص بيه تلقائي، جنب "🔧 واير/إيلاستيك/أكسسوارات". أي قسم جديد تضيفه من شاشة المخزون هيظهر زراره لوحده من غير أي تعديل. الكتابة الحرة في الخانات دلوقتي متعملش أي خصم خالص.</div>
        ...
        <div><b>Bonding Upper/Lower:</b> دلوقتي بزراير "🆕 Bonding Upper" و"🆕 Bonding Lower" تحت، مش بكتابة كلمة — Upper بيسألك نوع البراكيت (ROTH/MBT) ثم Tube أو Appliance للضروس، بيخصم سلك 0.012 تلقائي، ويسجل "حالة جديدة" للمريض ده الشهر ده. Lower بيسألك Tube أو Appliance بس، من غير ما يلمس نوع البراكيت أو السلك أو حالة جديدة</div>
        ...
        <div><b>Incisors Retraction:</b> اكتبها في خانة upper يتسجل Upper، وفي خانة lower يتسجل Lower — من غير ما تكتب كلمة upper/lower خالص. لو كتبتها في خانة both من غير ما تحدد جهة هتتسجل فوق وتحت مع بعض؛ ولو حبيت تحدد جهة واحدة بس من خانة both اكتب upper أو lower في نفس السطر</div>
        ...
        <div><b>Midline Correction:</b> بنفس منطق الـ Incisors بالظبط — خانة upper تسجل فوق، خانة lower تسجل تحت، وخانة both من غير تحديد تسجل الاتنين</div>
        <div><b>Firing:</b> بنفس منطق الـ Incisors بالظبط — خانة upper تسجل فوق، خانة lower تسجل تحت، وخانة both من غير تحديد تسجل الاتنين</div>
      </div>
    </div>
  `;
...
function attachMonthlyHandlers(){
  ...
  ['upper','lower','both'].forEach(part=>{
    document.querySelectorAll(`.monthly-done-${part}`).forEach(el=>{
      el.onchange = async () => {
        ...
        // stage-tagging runs FIRST and has no confirmation popup for Incisors Retraction/Midline,
        // so it always registers immediately — it used to run last, after scanAndDeductKeywords,
        // which shows a blocking "confirm deduction?" modal whenever the text also matches an
        // inventory keyword (e.g. a wire size like "0.16"); that modal delayed/interfered with
        // this step if the user moved on before answering it.
        await scanAdvancedStages(id, entry.doneUpper, entry.doneLower, entry.doneBoth);
  ...
  document.querySelectorAll('.bracket-action-btn').forEach(el=>{
    el.onclick = () => handleBracketAction(el.dataset.id);
  });
  document.querySelectorAll('.bonding-upper-action-btn').forEach(el=>{
    el.onclick = () => handleBondingUpperAction(el.dataset.id);
  });
  document.querySelectorAll('.bonding-lower-action-btn').forEach(el=>{
    el.onclick = () => handleBondingLowerAction(el.dataset.id);
  });
