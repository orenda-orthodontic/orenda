...
function attachGlobalLogoutButton(){
  ...
  // visible Undo/Redo — same actions as Ctrl+Z / Ctrl+Shift+Z below, for touch devices with no
  // physical keyboard. Placed on the opposite (right) side so they never collide with the
  // notification bell / hamburger menu buttons that live on the left.
  const undoBtn = document.createElement('button');
  undoBtn.id = 'globalUndoBtn';
  undoBtn.textContent = '↶ تراجع';
  undoBtn.className = 'secondary small';
  undoBtn.title = 'تراجع (Ctrl+Z)';
  undoBtn.style.cssText = 'position:fixed;top:10px;right:10px;z-index:999;';
  undoBtn.onclick = () => performUndo();
  document.body.appendChild(undoBtn);

  const redoBtn = document.createElement('button');
  redoBtn.id = 'globalRedoBtn';
  redoBtn.textContent = 'إعادة ↷';
  redoBtn.className = 'secondary small';
  redoBtn.title = 'إعادة (Ctrl+Shift+Z)';
  redoBtn.style.cssText = 'position:fixed;top:10px;right:80px;z-index:999;';
