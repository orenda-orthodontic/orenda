...
function renderAllClinicsAnalysisView(){
  const month = state.allClinicsAnalysisMonth || currentMonthStr();
  const data = state.allClinicsAnalysisData;

  let bodyHtml;
  if(state.allClinicsAnalysisLoading || !data || data.month !== month){
    bodyHtml = `<div class="placeholder" style="margin-top:10px;">جارِ تحليل كل العيادات...</div>`;
  } else {
    ...
    const tableRows = rows.length ? rows.map((r,idx)=>`
      <tr${idx===0 && r.income>0 ? ' style="background:#eaffef;"' : ''}>
    ...
    bodyHtml = `
      ...
      <div class="card" style="margin-bottom:14px;">
        ...
        <div class="placeholder" style="text-align:right;padding:8px;margin-bottom:10px;">مرتبة حسب الدخل — 🏆 أقوى عيادة الشهر ده</div>
  ...
  return `
    <div class="breadcrumb">
      <span class="crumb" id="crumbClinics4">العيادات</span>
      <span class="sep">/</span>
      <span>تحليل كل العيادات مجمعة</span>
    </div>
    <div class="card">
      <div class="row" style="justify-content:space-between;flex-wrap:wrap;">
        <div class="section-title" style="margin:0;">تحليل شهر ${monthLabelAr(month)} — كل العيادات</div>
        <input type="month" id="allClinicsAnalysisMonthInput" value="${escapeHtml(month)}">
      </div>
    </div>
    ${bodyHtml}
  `;
...
function attachAllClinicsAnalysisHandlers(){
  document.getElementById('crumbClinics4').onclick = () => { state.view = 'clinics'; render(); };
  const monthInput = document.getElementById('allClinicsAnalysisMonthInput');
  if(monthInput){
    monthInput.onchange = async () => {
      state.allClinicsAnalysisMonth = monthInput.value;
      state.allClinicsAnalysisLoading = true;
      render();
      await loadAllClinicsAnalysisData(state.allClinicsAnalysisMonth);
      render();
    };
  }
}
