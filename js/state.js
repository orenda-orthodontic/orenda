...
let state = {
  ...
  globalClinicCaseCounts: {}, // clinicId -> active (non-debonded) case count — computed alongside the alert scan at boot (scanAllClinicsForAlerts), powers the "المرضى" badge on each clinic tile (V2.6 Dashboard)
  globalFinanceSummary: { caseValueCents: 0, collectedCents: 0, remainingCents: 0, overdueCents: 0 }, // across every active patient in every clinic — computed alongside the alert scan at boot; powers the Dashboard's Finance summary strip (V2.7)
  globalFinanceBreakdown: {}, // clinicId -> {clinicName, caseValueCents, collectedCents, remainingCents, patients:[{patientId,name,caseValueCents,collectedCents,remainingCents,status:'paid'|'partial'|'overdue'}]} — same scan, powers the Clinic → Patient payment-history drill-down modal (V2.7)
  financeBreakdownExpandedClinicId: null, // which clinic is expanded in the finance drill-down modal (accordion — one at a time)
  globalPatientsIndex: [], // [{id, name, clinicId, clinicName}] — every patient across every clinic, built alongside the alert scan at boot; powers Global Search (Ctrl+K, V2.8) with no extra fetch
  ...
  customStageTypes: [] // [{id, label}] — global, doctor-added treatment-stage types beyond the fixed TREATMENT_STAGES list, shared across every patient/clinic
};
