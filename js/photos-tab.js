...
async function toggleSocialFlag(section, visitId, category, slotId){
  ...
  await savePatientFile(state.currentPatientId, stripHelperFields(pFile));
  await logActivity('social_flag', `${s.social ? 'علّم' : 'شال علامة'} صورة (${section}/${category}/${slotId}) ${s.social ? 'للسوشيال' : 'السوشيال'} لمريض ${patientNameForLog()}`);
  render();
}
...
function renderPhotosTab(){
  const file = state.currentPatientFile;
  const photos = ensurePhotos(file);
  // scoped per patient (not a single global) — switching tabs and back for the SAME patient keeps
  // whichever sub-tab (before/during/after) was open, but opening a DIFFERENT patient's Photos
  // tab always starts fresh on "قبل" instead of carrying over whatever the last patient had open
