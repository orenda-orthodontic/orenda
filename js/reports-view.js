...
function renderReportsView(){
  ...
  const workDaysCount = (state.reportsWorkDays !== null && state.reportsWorkDays !== undefined) ? state.reportsWorkDays : null;
...
async function loadReportsData(){
  ...
  // Reports can be opened directly from the top nav without ever visiting Clinics/Inventory this
  // session, so state.inventory might still be empty — load it here so the low-stock count in the
  // overview strip (and the inventory report itself) is accurate instead of silently showing 0.
  await loadInventory();
