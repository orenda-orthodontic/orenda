...
// بيجيب كذا مفتاح مستقل (مش patient file واحد) في نداء واحد بدل نداء منفصل لكل مفتاح — نفس فكرة
// batchLoadPatientFiles (clinic-features.js) لكن لمفاتيح عامة زي clinics/inventory/... — ده اللي
// بيقصّر وقت فتح البرنامج، لأن boot() كان بيستنى 4 نداءات متتالية قبل أول رسمة على الشاشة.
// عند فشل النداء المجمّع، بيرجع لنفس أسلوب getData العادي مفتاح مفتاح (fallback آمن).
// مهم: بيجيب updated_at كمان ويسجّله في _lastKnownUpdatedAt لأي مفتاح من CONCURRENCY_KEYS —
// زي ما idbGet بالظبط بيعمل — عشان أول حفظ لـ clinics/inventory/inventoryCategories/
// customStageTypes بعد الفتح يفضل محمي بالـ compare-and-swap بدل ما يرجع upsert عادي (auth.js).
// defaultsMap: { key: fallbackValue }  ->  يرجّع { key: value }
async function getDataBatch(defaultsMap){
  const keys = Object.keys(defaultsMap);
  const out = {};
  try{
    await ensureFreshSession();
    const quoted = keys.map(k => '"' + k + '"');
    const res = await fetch(SUPABASE_REST + '?key=in.(' + quoted.join(',') + ')&select=key,value,updated_at', { headers: supabaseHeaders() });
    ...
    keys.forEach(k => {
      const row = byKey[k];
      const v = row ? row.value : defaultsMap[k];
      out[k] = v;
      _lastKnownValues[k] = deepClone(v);
      if(row && typeof isConcurrencyProtectedKey === 'function' && isConcurrencyProtectedKey(k)){
        _lastKnownUpdatedAt[k] = row.updated_at;
      }
    });
  }catch(e){
    console.error('getDataBatch failed, falling back to per-key getData', e);
    for(const k of keys) out[k] = await getData(k, defaultsMap[k]);
  }
  return out;
}
...
async function _setDataInner(key, value, opts){
  ...
  _lastKnownValues[key] = deepClone(value);
  try{
...
const CONCURRENCY_KEY_DEFAULTS = { inventory: [], clinics: [], inventoryCategories: [], orthoNotes: [] };
...
function undoKeyLabel(key){
  ...
  if(key === 'orthoNotes') return 'ملاحظاتي';
...
function applyRestoredValueToState(key, value){
  ...
  else if(key === 'orthoNotes'){ state.orthoNotes = value || []; }
...
function historyEntryMatchesScreen(entry){
  ...
  if(state.view === 'orthonotes') return entry.key === 'orthoNotes';
