...
function savePendingWritesMap(map){
  ...
  try{ localStorage.setItem(PENDING_WRITES_KEY, json); return true; }catch(e){ /* الكوتا — نفضّي مساحة تحت */ }
  // سجل الـ undo/redo بيتعاد بناؤه وملوش قيمة قدام تعديل لسه ما اتبعتش للسيرفر، فنضحّي بيه الأول
  // (redo الأول، وبعدين undo) ونحاول تاني بدل ما التعديل الحقيقي يضيع بصمت
  try{ localStorage.removeItem(REDO_STACK_KEY); localStorage.setItem(PENDING_WRITES_KEY, json); return true; }catch(e){ /* كمّل */ }
  try{ localStorage.removeItem(UNDO_STACK_KEY); localStorage.setItem(PENDING_WRITES_KEY, json); return true; }catch(e){
    console.error('could not persist pending-writes queue', e);
  }
  if(typeof toast === 'function') toast('⚠ مساحة التخزين على الجهاز امتلت — التعديلات اللي لسه ما اتبعتتش ممكن تضيع لو قفلت الصفحة. تأكد من النت.');
  return false;
}
...
async function _setDataInner(key, value, opts){
  ...
  // MUST be a deep clone, not the same reference as `value` — `value` shares nested
  // arrays/objects with the live, still-being-edited state (e.g. state.currentPatientFile), so
  // storing it by reference would let later in-place edits silently rewrite this "previous
  // snapshot" before it's ever used for an undo diff (see note above _lastKnownValues).
  // نسخ الباكاب الأوتوماتيك ضخمة (نسخة من قاعدة البيانات كلها) — مالهاش لازمة في الـ undo، ومينفعش
  // تتخزن في localStorage كطابور أوفلاين (بتعدي الكوتا وتوقّف تخزين تعديلات المرضى الحقيقية)
  const isBulkBackupKey = key.indexOf('dailybackup:') === 0 || key.indexOf('preimportsnapshot:') === 0;
  if(!isBulkBackupKey) _lastKnownValues[key] = deepClone(value);
  try{
    ...
    if(e && e.name === 'ReadNotConfirmedError'){
      // القراءة الأخيرة للمفتاح ده فشلت، فاللي في الميموري ممكن يكون نسخة فاضية/قديمة — منكتبهاش
      // فوق الأصلي، ومنطابرهاش أوفلاين (لو اتطابرت هتتكتب بعدين فوق البيانات الحقيقية)
      console.error('write refused (read had failed) for', key);
      discardUndoEntry(undoTs);
      toast('⚠ مقدرتش أقرأ البيانات دي من السيرفر، فمنعت الحفظ عشان مايتمسحش اللي متسجل. اعمل ريفرش وحاول تاني.');
      return false;
    }
    console.error('storage set failed', key, e);
    if(isBulkBackupKey){
      // مش بنطابر النسخة الاحتياطية ولا بنطلع توست "مفيش نت" — المحاولة الجاية هتحصل في أول
      // تشغيل تاني (lastAutoBackupDate ما اتحدّثش لما فشلت)، والفشل هنا مالوش علاقة بتعديلات المرضى
      return false;
    }
...
// Uses a blocking alertModal (not the 1.8s auto-dismiss toast) — a save that got silently
// discarded is exactly the kind of thing a person keeps typing right past if the only sign of
// it disappears in under 2 seconds; this forces them to actually see it before continuing.
const CONCURRENCY_KEY_DEFAULTS = { inventory: [], clinics: [], inventoryCategories: [] };
async function handleConcurrencyConflict(key){
  const msg = '⚠ ' + undoKeyLabel(key) + ' اتعدّل من جهاز/تاب تاني في نفس الوقت — آخر تعديل كتبته اتلغى عشان محدش يمسح تعديل التاني فوق التاني. اتحمّلت آخر نسخة محفوظة على السيرفر، لازم تكرر التعديل اللي عملته تاني.';
  if(typeof alertModal === 'function') await alertModal(msg, {title:'تعارض في الحفظ'});
  else toast(msg);
...
// IMPORTANT: state.currentPatientFile (and similar live state objects) get mutated IN PLACE
// all over the app (Object.assign(field, changes), arr.push(...), t.status = 'broken', etc.),
// and savePatientFile only does a SHALLOW copy before saving ({...file}) — nested arrays like
// diagnosisSections/monthlyLog/bracketMap keep the same reference. If _lastKnownValues[key]
// ever pointed at that same nested data (as it used to), every subsequent in-place edit would
// silently "leak backwards" into what's supposed to be the frozen previous-snapshot, so
// recordUndoBeforeWrite's diff would find no real change (edit lost) or — worse — push a
// prevValue that was already contaminated with later, unrelated edits (undo then wipes fields
// that were never touched by the action being undone, and section-detection breaks, which is
// also why undo could appear to fire on the wrong tab). Always store an independent deep clone
// here so it can never be mutated by anything else.
function deepClone(obj){
  if(obj === null || obj === undefined) return obj;
  if(typeof structuredClone === 'function'){
    try{ return structuredClone(obj); }catch(e){ /* fall through to JSON clone below */ }
  }
...
// كل خطوة undo فيها نسخة كاملة من ملف المريض — 40 خطوة على ملفات كبيرة كانت بتعدي كوتا الـ
// localStorage (~5MB) والكود كان بيبلع الخطأ فالتراجع بيقف بصمت. دلوقتي بنقص من أقدم الخطوات لحد
// ما الباقي يدخل في ميزانية معقولة، ولو المتصفح رفض برضه بنكمل نقص لحد ما يقبل.
const STACK_BUDGET_CHARS = 1500000;
function persistStackWithBudget(key, stack, label){
  const s = stack.slice();
  while(true){
    let json;
    try{ json = JSON.stringify(s); }catch(e){ console.error('could not serialise ' + label + ' stack', e); return false; }
    if(json.length > STACK_BUDGET_CHARS && s.length > 0){ s.shift(); continue; }
    try{ localStorage.setItem(key, json); return true; }
    catch(e){
      if(s.length > 0){ s.shift(); continue; }
      console.error('could not persist ' + label + ' stack', e);
      return false;
    }
  }
}
function saveUndoStack(stack){ persistStackWithBudget(UNDO_STACK_KEY, stack, 'undo'); }
function loadRedoStack(){
  try{ return JSON.parse(localStorage.getItem(REDO_STACK_KEY) || '[]'); }
  catch(e){ return []; }
}
function saveRedoStack(stack){ persistStackWithBudget(REDO_STACK_KEY, stack, 'redo'); }
...
const PATIENT_TAB_SECTIONS = {
  ...
  monthly: ['monthly','brackets','header'],
