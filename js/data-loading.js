...
async function loadClinics(){
  // كان 4 نداءات شبكة متتالية (كل واحد رحلة كاملة للسيرفر) قبل أول رسمة على الشاشة عند الفتح —
  // دلوقتي نداء واحد مجمّع (getDataBatch) بدل ما نستنى الأربعة واحد ورا التاني.
  const data = await getDataBatch({
    clinics: [],
  ...
  state.dismissedAlerts = data.dismissedAlerts;
  state.customStageTypes = data.customStageTypes;
  // clinic-owner photos are in the private bucket too — sign them up front so the clinics grid can show them
  try{ await refreshPhotoUrlCache(state.clinics.map(c => c.doctorPhoto && c.doctorPhoto.path)); }
  catch(e){ console.error('clinic photo signing failed', e); }
}

async function saveCustomStageTypes(){
  return await setData('customStageTypes', state.customStageTypes);
}
...
async function loadInventory(){
  // نفس فكرة loadClinics: نداء واحد مجمّع بدل 3 نداءات متتالية (inventory + deletedRevealTeeth +
  // inventoryCategories) — التلاتة مستقلين عن بعض، محتاجينهم مع بعض قبل أي منطق تحت.
  const data = await getDataBatch({
    inventory: [],
    deletedRevealTeeth: [],
  ...
  state.deletedRevealTeeth = data.deletedRevealTeeth;
  let dirty = false;
  ...
  state.inventoryCategories = data.inventoryCategories;
...
async function saveInventory(){
  return await setData('inventory', state.inventory);
}
async function saveDeletedRevealTeeth(){
  return await setData('deletedRevealTeeth', state.deletedRevealTeeth);
}
async function saveInventoryCategories(){
  return await setData('inventoryCategories', state.inventoryCategories);
}
async function loadPatientFile(patientId){
  ...
  if(!file.diagnosisSections || file.diagnosisSections.length === 0){
    ...
    if(migrateExtraOralFrontalFields(file.diagnosisSections)) dirty = true;
    if(migrateOldChiefComplaintSection(file)) dirty = true;
  }
  ...
  if(!file.notes) file.notes = []; // {id, text, date} — shown highlighted on the Overview tab
  if(file.accountSettled === undefined) file.accountSettled = false;
  if(file.accountSettledDate === undefined) file.accountSettledDate = null;
...
async function savePatientFile(patientId, file){
  return await setData('patientfile:' + patientId, file);
}
...
function buildDefaultDiagnosisSections(){
  ...
  return [
    ...
    mk('Space Management / Anchorage', {
      fields: [
        ['Space management', ['Stripping', 'Proclination', 'Expansion', 'Distalization', 'Extraction']],
        'Anchorage',
        ['Tooth out of wire', ['Yes', 'No']],
        ['Wire sequence', ['Single', 'Double']],
        ['Bite turbo', ['Yes', 'No']]
      ]
    }),
...
function migrateExtraOralFrontalFields(sections){
  ...
  if(intraOral){
    ...
    addNewField(iFrontal, 'Frontal Cross Bite', ['Yes', 'No']);
    ...
    addNewField(intraOral, 'Oral Hygiene', ['Good', 'Fair', 'Poor']);
  }
  ...
  const spaceManagement = sections.find(s => s.title === 'Space Management / Anchorage');
  addOptionsToField(spaceManagement, 'Space management', ['Stripping', 'Proclination', 'Expansion', 'Distalization', 'Extraction']);
  addOptionsToField(spaceManagement, 'Tooth out of wire', ['Yes', 'No']);
  addOptionsToField(spaceManagement, 'Wire sequence', ['Single', 'Double']);
  addOptionsToField(spaceManagement, 'Bite turbo', ['Yes', 'No']);

  return changed;
}

// دمج قسم "Chief Complaint" القديم (كان section منفصل جوه شيت التشخيص، فاضي غالبًا غير النص
// الحر) في الصندوق الجديد المخصص فوق شيت التشخيص (file.chiefComplaint) — بينقل أي نص كان
// مكتوب فيه قبل كده (لو الصندوق الجديد لسه فاضي) بدل ما يضيع، وبعدين يشيل القسم القديم عشان
// مايتكررش مع الصندوق الجديد.
function migrateOldChiefComplaintSection(file){
  const idx = (file.diagnosisSections||[]).findIndex(s => s.title === 'Chief Complaint');
  if(idx === -1) return false;
  const oldText = ((file.diagnosisSections[idx].content) || '').trim();
  if(oldText && !file.chiefComplaint){
    file.chiefComplaint = oldText;
  }
  file.diagnosisSections.splice(idx, 1);
  return true;
}
