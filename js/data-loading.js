...
async function loadClinics(){
  ...
  state.customStageTypes = await getData('customStageTypes', []);
...
async function loadPatientFile(patientId){
  ...
  // القراءة فشلت (timeout/500) — getData رجّع الملف الفاضي الافتراضي. لو كملنا هيتعرض للدكتور ملف
  // مريض فاضي وممكن يتحفظ فوق الأصلي، فنوقف هنا ونعرض شاشة الخطأ بدل كده.
  if(typeof _readFailedKeys !== 'undefined' && _readFailedKeys['patientfile:' + patientId]){
    throw new Error('تعذر قراءة ملف المريض من السيرفر — ممكن يكون ضغط مؤقت، حاول تاني');
  }
  let dirty = false;
