// wire-accessories-modal.js — نافذة اختيار الواير (فوق/تحت) والأكسسوارات لتسجيلات المتابعة
// الشهرية، بدل كتابتهم كنص حر واستنى الكود يتعرف عليهم. بيحل مشكلة إن كل دكتور بيكتب بصياغة
// مختلفة (wire 14 / سلك 16 / NiTi...) فيفشل الخصم بصمت — هنا الاختيار من قايمة فعلية، مضمون
// 100%. الصنف اللي خلص من المخزن يفضل ظاهر في القايمة لكن مش قابل للاختيار (disabled)، مش
// بيختفي، عشان الدكتور يعرف إنه موجود بس محتاج توريد.
// باقي أنواع الخصم (bonding upper/lower، rebonding/فص، tube، bracket، elastics) تفضل زي ما هي
// بالـ keyword في keyword-deduction.js — الملف ده بيغطي الواير والأكسسوارات بس.

function wamOptionsHtml(items){
  return items.map(i => {
    const qty = parseFloat(i.qty) || 0;
    return `<option value="${i.id}" ${qty <= 0 ? 'disabled' : ''}>${escapeHtml(i.name)} — متاح: ${qty}</option>`;
  }).join('');
}

// returns { wireUpperId, wireLowerId, accessories:[itemId,...] } or null if cancelled/empty
  ...
  return new Promise(resolve => {
    ...
    bg.innerHTML = `
      <div class="modal" style="max-width:420px;">
        <h3>واير وأكسسوارات — المتابعة دي</h3>
        <div style="max-height:60vh;overflow-y:auto;padding-inline-end:4px;">
          ...
          ${(!wireItems.length && !accessoryItems.length) ? `<div class="placeholder" style="margin-top:10px;">مفيش أصناف مسجلة في قسم "وايرز" أو "أكسسوريز" بالمخزون — ضيفهم الأول من شاشة المخزون.</div>` : ''}
        </div>
    ...
    function renderAccessoryChips(){
      const listEl = document.getElementById('wamAccessoryList');
      listEl.innerHTML = pickedAccessories.map((a, idx) => `
      ...
      listEl.querySelectorAll('[data-idx]').forEach(btn => {
        btn.onclick = () => { pickedAccessories.splice(parseInt(btn.dataset.idx, 10), 1); renderAccessoryChips(); };
      });
    }
    ...
    document.getElementById('wamConfirmBtn').onclick = () => {
      ...
      if(!wireUpperId && !wireLowerId && !pickedAccessories.length){ cleanup(null); return; }
      cleanup({ wireUpperId, wireLowerId, accessories: pickedAccessories.map(a => a.itemId) });
    };
  });
}
