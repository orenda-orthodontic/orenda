...
async function handleBondingUpperAction(entryId){
  ...
  await logActivity('bonding_upper', `Bonding Upper (${bracketSystemLabel(bracketPick)}) + حالة جديدة — ${bondingAttachmentLine(attachment)} — مريض ${patientNameForLog()}`);
...
async function handleBondingLowerAction(entryId){
  ...
  await savePatientFile(state.currentPatientId, stripHelperFields(file));
  await logActivity('bonding_lower', `Bonding Lower — ${bondingAttachmentLine(attachment)} — مريض ${patientNameForLog()}`);
...
async function markNewCaseManually(){
  ...
  await savePatientFile(state.currentPatientId, stripHelperFields(file));
  await logActivity('new_case_manual', `سجّل حالة جديدة بتاريخ ${formatDateAr(date)} لمريض ${patientNameForLog()}`);
...
async function markCaseDebonded(){
  ...
  await savePatientFile(state.currentPatientId, stripHelperFields(file));
  await logActivity('debond_case', `سجّل فك الحالة بتاريخ ${formatDateAr(date)} لمريض ${patientNameForLog()}`);
...
async function editCaseStartDate(){
  ...
  await savePatientFile(state.currentPatientId, stripHelperFields(file));
  await logActivity('edit_case_start', `عدّل تاريخ بداية الحالة إلى ${formatDateAr(date)} لمريض ${patientNameForLog()}`);
