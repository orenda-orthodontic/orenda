...
function applyBondingAttachment(e, attachment, entryId, archTag){
  if(attachment.kind === 'tube'){
    attachment.tubeItems.forEach(item=>{
      ...
      e.materialsUsed.push({ id: uid(), itemId: item.id, itemName: item.name, itemType: item.type || '', qty: 1, matchKey: 'bonding:'+archTag+':'+item.id+':'+entryId, source:'keyword' });
    });
    ...
    e.materialsUsed.push({ id: uid(), itemId: attachment.applianceItem.id, itemName: attachment.applianceItem.name, itemType: attachment.applianceItem.type || '', qty: 1, matchKey: 'bonding:'+archTag+':appliance:'+entryId, source:'keyword' });
  } else {
...
async function handleBondingUpper(file, e, entryId, text){
  ...
  e.materialsUsed.push({ id: uid(), itemId: bracketItem.id, itemName: bracketItem.name, itemType: bracketItem.type || '', qty: 1, matchKey: 'bonding:upper:bracket:'+entryId, source:'keyword' });

  applyBondingAttachment(e, attachment, entryId, 'upper');

  wireItem.qty = Math.max(0, (parseFloat(wireItem.qty)||0) - 1);
  e.materialsUsed.push({ id: uid(), itemId: wireItem.id, itemName: wireItem.name, itemType: wireItem.type || '', qty: 1, matchKey: 'bonding:upper:wire:'+entryId, source:'keyword' });
