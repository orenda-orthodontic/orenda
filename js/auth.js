...
// لو أكتر من نداء بيحصل في نفس اللحظة (مثلاً loadPatients وloadInventory شغالين بالتوازي) وكلاهم
// لقوا الجلسة قربت تخلص، بيتشاركوا نفس عملية الـ refresh (بدل ما كل واحد يبعت طلب refresh منفصل
// بنفس الـ refresh_token القديم — Supabase بيلغي القديم بعد أول استخدام، فالنداء التاني كان ممكن
// يفشل ويطلع "الجلسة انتهت" غلط).
let _refreshInFlight = null;
async function ensureFreshSession(){
  if(!authSession) return false;
  if(Date.now() > authSession.expires_at - 60000){
    if(!_refreshInFlight){
      _refreshInFlight = supabaseRefreshSession().finally(()=>{ _refreshInFlight = null; });
    }
...
const CONCURRENCY_KEYS = ['inventory', 'clinics', 'inventoryCategories', 'customStageTypes', 'orthoNotes'];
