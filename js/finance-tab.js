...
function printPatientStatement(){
  ...
  const hasManualRemaining = file.financeRemaining !== undefined && file.financeRemaining !== null;
  const remaining = hasManualRemaining ? (parseFloat(file.financeRemaining) || 0) : (caseValue - paidSum);
...
function renderFinanceTab(){
  ...
  const paidSum = payments.reduce((s,p)=> s + (parseFloat(p.amount)||0), 0);
  const caseValue = total + extrasSum;
  const computedRemaining = caseValue - paidSum;
  const hasManualRemaining = file.financeRemaining !== undefined && file.financeRemaining !== null;
  const remaining = hasManualRemaining ? (parseFloat(file.financeRemaining) || 0) : computedRemaining;
  ...
  return `
    ...
    <div class="row" style="gap:16px;">
      ...
      <div class="field" style="max-width:240px;">
        <label>المتبقي ${hasManualRemaining ? '(مُدخل يدويًا)' : '(محسوب تلقائيًا — عدّله يدويًا للحالات اللي بدأت قبل النظام)'}</label>
        <input type="number" id="financeRemainingInput" value="${remaining}">
      </div>
    </div>
...
async function toggleAccountSettled(){
  ...
  if(file.accountSettled){
    ...
    file.accountSettledDate = todayStr();
  }
  await savePatientFile(state.currentPatientId, stripHelperFields(file));
...
async function updateFinanceRemaining(value){
  const file = state.currentPatientFile;
  file.financeRemaining = parseFloat(value) || 0;
  await savePatientFile(state.currentPatientId, stripHelperFields(file));
  await logActivity('set_remaining', `حدّد المتبقي يدويًا (${file.financeRemaining} جنيه) لمريض ${patientNameForLog()}`);
  toast('اتحفظ');
  render();
}
...
function openAddPaymentModal(){
  ...
  document.getElementById('savePaymentBtn').onclick = async () => {
    ...
    if(file.financeRemaining !== undefined && file.financeRemaining !== null){
      file.financeRemaining = (parseFloat(file.financeRemaining) || 0) - amount;
    }
    await savePatientFile(state.currentPatientId, stripHelperFields(file));
...
function openEditPaymentModal(id){
  ...
  document.getElementById('saveEditPaymentBtn').onclick = async () => {
    ...
    if(amount <= 0){ toast('اكتب مبلغ صحيح'); return; }
    const oldAmount = parseFloat(p.amount) || 0;
    if(file.financeRemaining !== undefined && file.financeRemaining !== null){
      file.financeRemaining = (parseFloat(file.financeRemaining) || 0) + oldAmount - amount;
    }
...
async function deletePayment(id){
  ...
  if(p && file.financeRemaining !== undefined && file.financeRemaining !== null){
    file.financeRemaining = (parseFloat(file.financeRemaining) || 0) + (parseFloat(p.amount) || 0);
  }
  await savePatientFile(state.currentPatientId, stripHelperFields(file));
...
async function deleteFinanceExtra(id){
  ...
  if(removedExtra && file.financeRemaining !== undefined && file.financeRemaining !== null){
    file.financeRemaining = (parseFloat(file.financeRemaining) || 0) - (parseFloat(removedExtra.amount) || 0);
  }
