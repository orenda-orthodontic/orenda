...
function renderOverviewTab(){
  ...
  // سجل الحضور — جه كام مرة وامتى، مجاش كام مرة وامتى وليه (مبني على نفس علامة "❌ مجاش"
  // اللي بتتحط يدوي في المتابعة الشهرية — مش استنتاج من فراغ في التسجيل)
  const dateLoggedEntries = (file.monthlyLog||[]).filter(e=>e.date);
  const cameEntries = dateLoggedEntries.filter(e=>!e.missed).sort((a,b)=>a.date.localeCompare(b.date));
  const missedVisitEntries = dateLoggedEntries.filter(e=>e.missed).sort((a,b)=>a.date.localeCompare(b.date));
  ...
  return `
    ...
    <div class="overview-grid">
      ...
      <div class="overview-card">
        ...
        <div class="overview-row"><span class="ov-lbl">جه</span><span class="ov-val">${cameEntries.length} مرة</span></div>
        ${cameEntries.length ? `<div style="font-size:12px;color:var(--muted);margin-bottom:8px;line-height:1.8;">${cameEntries.map(e=>escapeHtml(formatDateAr(e.date))).join('، ')}</div>` : ''}
        <div class="overview-row"><span class="ov-lbl">مجاش</span><span class="ov-val ${missedVisitEntries.length?'ov-alert':''}">${missedVisitEntries.length} مرة</span></div>
        ${missedVisitEntries.length ? `<div style="font-size:12px;color:var(--muted);line-height:1.8;">${missedVisitEntries.map(e=>escapeHtml(formatDateAr(e.date)) + (e.missedReason ? ` (${escapeHtml(e.missedReason)})` : '')).join('، ')}</div>` : ''}
      </div>
    </div>
  `;
}
