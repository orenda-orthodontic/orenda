    ...
    <div class="card">
      <div class="patient-header">
        <div>
          ...
          ${computeUnpaidMonths(file) >= UNPAID_MONTHS_ALERT_THRESHOLD ? `<div class="psub" style="margin-top:4px;"><span class="appt-badge overdue stock-alert-blink">💰 مدفعش من ${computeUnpaidMonths(file)} شهر</span></div>` : ''}
...
function renderOverviewTab(){
  ...
  // سجل الحضور — قايمة زمنية واحدة: جه وقتها اتعمل إيه، أو مجاش وكان المفروض يتعمل إيه —
  // اللي "كان المفروض" ده مش خطة الشهر المفقود نفسه (مفيش حاجة اتسجلت فيه أصلاً)، ده خطة
  // "الشهر الجاي" اللي كانت مكتوبة في آخر زيارة حقيقية قبله (يعني لو جه شهر 8 ومجاش شهر 9،
  // اللي هيتكتب جمب شهر 9 هو خطة شهر 8 للشهر اللي بعده)
  function shortText(t, max){
    t = (t||'').replace(/\s+/g, ' ').trim();
    return t.length > max ? t.slice(0, max).trim() + '…' : t;
  }
  function doneTextFor(e){
    return [e.doneUpper, e.doneLower, e.doneBoth !== undefined ? e.doneBoth : e.done].filter(Boolean).join(' | ');
  }
  function planTextFor(e){
    return [e.planUpper, e.planLower, e.planBoth !== undefined ? e.planBoth : e.plan].filter(Boolean).join(' | ');
  }
  const dateLoggedEntries = (file.monthlyLog||[]).filter(e=>e.date).sort((a,b)=>a.date.localeCompare(b.date));
  const attendanceRows = dateLoggedEntries.map((e, idx) => {
    if(e.missed){
      const prevVisit = dateLoggedEntries.slice(0, idx).reverse().find(p=>!p.missed);
      return { date: e.date, missed: true, detail: prevVisit ? shortText(planTextFor(prevVisit), 80) : '', reason: e.missedReason };
    }
    return { date: e.date, missed: false, detail: shortText(doneTextFor(e), 80) };
  }).reverse(); // أحدث حاجة فوق
      ...
      <div class="overview-card">
        ...
        ${attendanceRows.length ? `
          <div style="margin-top:8px;max-height:220px;overflow-y:auto;">
            ${attendanceRows.map(r => r.missed ? `
              <div style="font-size:12px;line-height:1.7;padding:5px 0;border-bottom:1px solid var(--border);">
                <b>${escapeHtml(formatDateAr(r.date))}</b> — <span style="color:var(--red);">❌ مجاش${r.reason ? ' (' + escapeHtml(r.reason) + ')' : ''}</span>
                ${r.detail ? `<div style="color:var(--muted);margin-top:2px;">المفروض كان يتعمل: ${escapeHtml(r.detail)}</div>` : ''}
              </div>
            ` : `
              <div style="font-size:12px;line-height:1.7;padding:5px 0;border-bottom:1px solid var(--border);">
                <b>${escapeHtml(formatDateAr(r.date))}</b> — جه${r.detail ? `: ${escapeHtml(r.detail)}` : ''}
              </div>
            `).join('')}
          </div>
        ` : `<div class="placeholder">لسه مفيش متابعة مسجلة</div>`}
