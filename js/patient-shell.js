...
// آخر مرحلة علاج اتسجلت (label + date) — مستخرجة من stageCounts (hygiene-stages-bracketmap.js)
// و allTreatmentStages() (followup-tab.js). موحّدة هنا عشان renderPatientView (الهيدر/الملخص)
// وrenderOverviewTab (تبويب النظرة العامة) يستخدموا نفس الحساب بدل ما كل واحد يعمل نسخته
// الخاصة (كان فيه نسخة جوه renderOverviewTab بس كـ local vars، فـ renderPatientView كان بيقرا
// عليها بالغلط وهي مش موجودة في الـ scope بتاعه -> ReferenceError: lastStageLabel is not defined).
function computeLastStage(file){
  ...
  return { lastStageLabel, lastStageDate };
}

// إجمالي الحالة/المدفوع/الباقي — نفس السبب والحل بتاع computeLastStage فوق: كانت متحسوبة كـ
// local vars جوه renderOverviewTab بس، وrenderPatientView (الهيدر) كان بيستخدم `remaining`
// من غير ما يكون معرّف في الـ scope بتاعه -> ReferenceError: remaining is not defined.
function computeCaseFinance(file){
  ...
  return { total, extrasSum, paidSum, caseValue, remaining };
}
...
function renderOverviewTab(){
  ...
  const { total, extrasSum, paidSum, caseValue, remaining } = computeCaseFinance(file);
