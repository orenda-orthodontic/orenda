...
// الباقي على المريض — نفس منطق تبويب الحسابات بالظبط: القيمة اليدوية لو الدكتور حددها، وإلا
// (إجمالي الحالة + الرسوم الإضافية - المدفوع). مستخدمة في الهيدر وفي كارت "نظرة عامة" عشان
// الرقمين ميختلفوش عن بعض.
function patientRemainingAmount(file){
  const caseValue = (parseFloat(file.financeTotal) || 0) + (file.financeExtras||[]).reduce((s,e)=>s+(parseFloat(e.amount)||0),0);
  const paid = (file.payments||[]).reduce((s,p)=>s+(parseFloat(p.amount)||0),0);
  const manual = (file.financeRemaining !== undefined && file.financeRemaining !== null) ? (parseFloat(file.financeRemaining) || 0) : null;
  return Math.round((manual !== null ? manual : (caseValue - paid)) * 100) / 100;
}
...
async function renderPatientView(){
  ...
  // آخر مرحلة علاج اتسجلت — لازم تتحسب هنا لأن نفس المتغيرات جوه renderOverviewTab بس (scope تاني)
  let lastStageLabel = '', lastStageDate = '';
  try{
    const heroStageCounts = computeStageCounts(file);
    allTreatmentStages().forEach(st=>{
      (heroStageCounts[st.id]||[]).forEach(e=>{
        if(!lastStageDate || (e.date||'') > lastStageDate){ lastStageDate = e.date||''; lastStageLabel = st.label; }
      });
    });
  }catch(err){ console.error('last stage calc failed', err); }
  const remaining = patientRemainingAmount(file);
...
function renderOverviewTab(){
  ...
  const remaining = patientRemainingAmount(file); // بيحترم المتبقي اليدوي — نفس رقم الهيدر وتبويب الحسابات
