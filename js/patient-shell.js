...
async function renderPatientView(){
  ...
  // الباقي للهيدر — نفس منطق تبويب الحسابات: القيمة اليدوية لو متسجلة، وإلا (الإجمالي + الرسوم - المدفوع)
  const heroCaseValue = (parseFloat(file.financeTotal) || 0) + (file.financeExtras||[]).reduce((s,e)=>s+(parseFloat(e.amount)||0),0);
  const heroPaid = (file.payments||[]).reduce((s,p)=>s+(parseFloat(p.amount)||0),0);
  const heroManualRemaining = (file.financeRemaining !== undefined && file.financeRemaining !== null) ? (parseFloat(file.financeRemaining) || 0) : null;
  const remaining = Math.round((heroManualRemaining !== null ? heroManualRemaining : (heroCaseValue - heroPaid)) * 100) / 100;
